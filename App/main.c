...
lcore_rx(struct lcore_params *p)
{
	...
	while (!quit_signal_rx) {
		...
		uint16_t sent = rte_ring_enqueue_burst(in_ring,
...
static int
lcore_tx(struct rte_ring *out_r)
{
	...
	while (!quit_signal) {
		...
		RTE_ETH_FOREACH_DEV(port) {
			...
			const uint16_t nb_rx = rte_ring_dequeue_burst(out_r,
...
lcore_worker(struct lcore_params *p)
{
	...
	struct rte_ring *in_r = p->rx_ring;
	struct rte_ring *out_r = p->tx_ring;
	...
	while (!quit_signal_work) {
		const uint16_t nb_rx = rte_ring_dequeue_burst(in_r,
				(void *)buf, BURST_SIZE*1, NULL);
		if (unlikely(!nb_rx))
			continue;

		/* Do a little bit of work for each packet */
		for (i = 0; i < num; i++) {
			uint64_t t = rte_rdtsc()+ 0; //100;
...
main(int argc, char *argv[])
{
	...
	rx_ring = rte_ring_create("rx_ring", SCHED_RX_RING_SZ,
			rte_socket_id(), RING_F_SC_DEQ | RING_F_SP_ENQ);
	if (rx_ring == NULL)
		rte_exit(EXIT_FAILURE, "Cannot create output ring\n");
	...
	RTE_LCORE_FOREACH_SLAVE(lcore_id) {
		...
		*p = (struct lcore_params){worker_id++, rx_ring,
			tx_ring, mbuf_pool};
	...
	if (!pr)
		rte_panic("malloc failure\n");
	*pr = (struct lcore_params){worker_id++, rx_ring,
		tx_ring, mbuf_pool};
